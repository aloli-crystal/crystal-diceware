...
describe CrystalDiceware do
  describe ".generate" do
    it "produit une passphrase de N mots séparés par des espaces" do
      phrase = CrystalDiceware.generate(words: 7, language: :eff_long)
      phrase.split(' ').size.should eq(7)
    end

    it "respecte le séparateur custom" do
      phrase = CrystalDiceware.generate(words: 5, language: :eff_long, separator: "-")
      phrase.split('-').size.should eq(5)
    end

    it "rejette words ≤ 0" do
      expect_raises(CrystalDiceware::Error, /words doit/) do
        CrystalDiceware.generate(words: 0)
      end
      expect_raises(CrystalDiceware::Error, /words doit/) do
        CrystalDiceware.generate(words: -3)
      end
    end
    ...
    it "mode :manual sans rolls lève une erreur" do
      expect_raises(CrystalDiceware::Error, /nécessite/) do
        CrystalDiceware.generate(words: 3, source: :manual)
      end
    end

    it "mode :manual avec mauvais nombre de rolls lève une erreur" do
      expect_raises(CrystalDiceware::Error, /3 éléments/) do
        CrystalDiceware.generate(
    ...
    it "source inconnue lève une erreur" do
      expect_raises(CrystalDiceware::Error, /source inconnue/) do
        CrystalDiceware.generate(words: 3, source: :bogus)
      end
    end
  end
  ...
  describe ".entropy" do
    it "= words × log₂(7776) pour une liste de 7776" do
      bits = CrystalDiceware.entropy(words: 7, wordlist: :eff_long)
      bits.should be_close(90.47, 0.01)
    end

    it "résultat double pour 14 mots" do
      bits14 = CrystalDiceware.entropy(words: 14, wordlist: :eff_long)
      bits7 = CrystalDiceware.entropy(words: 7, wordlist: :eff_long)
